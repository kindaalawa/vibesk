 ...
 * @returns {Promise<Array>} A promise that resolves to an array of addresses for the given user.
 * @throws {Error} If there's an error during the database query.
 */
const getAddressByUserId = async (user_ID) => {
    try {
        let sql = `SELECT * FROM address WHERE user_ID = ?`;
        const address = await query(sql, [user_ID]);
        return address;
    } catch (error) {
        throw new Error(error);
    }
 ...
 * @param {number} user_ID - The ID of the user associated with the address.
 ...
 * @throws {Error} If there's an error during the database query.
 */
const insertAddress = async (user_ID, region, street, building, floor, moredetails) => {
    try {
        let sql = `INSERT INTO address 
            (user_ID, region, street, building, floor, moredetails)
            VALUES (?, ?, ?, ?, ?, ?)`;
        const result = await query(sql, [user_ID, region, street, building, floor, moredetails]);
